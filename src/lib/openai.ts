...
export async function openAIWithStreamGptsToml(
  ...
  try {
    ...
    console.error("Catch block in openAIWithStreamGptsToml:", e);
...
export async function dallE_gptsToml(
  ...
  try {
    ...
    console.log(
      `Curl Command: curl -X POST "${url}" -H "Authorization: Bearer ${
        options.apiKey
