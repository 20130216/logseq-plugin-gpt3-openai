...
export async function openAIWithStreamGptsID(
  ...
  try {
    ...
    const body = {
      ...
      frequency_penalty: 0,
      presence_penalty: 0,
      model: options.gpts,  // 单独增加；核心：把gpts-id传入在此，作为调用model的参数 "model": "gpt-4-gizmo-g-2fkFE8rbu",
      stream: true,
    };
...
export async function readImageURL(url: string, openAiOptions: OpenAIOptions): Promise<string> {
  const apiKey = openAiOptions.apiKey;
  const baseUrl = openAiOptions.completionEndpoint ? openAiOptions.completionEndpoint : "https://api.shubiaobiao.cn/v1";  
