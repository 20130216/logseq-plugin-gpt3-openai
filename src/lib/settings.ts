...
export const settingsSchema: SettingSchemaDesc[] = [
  ...
  },
  {
    ...
    description:
      "推荐：<br/>1.默认的gpt-4o-mini是性价比最高的OpenAI的模型；<br/>2.ChatGPT-4o-latest是在世界范围内长期霸榜多个大模型评测榜单榜首的模型（虽略贵，但效果爆表）", // "Recommended models and their features"
  },
  {
  ...
  },
  {
    ...
    type: "string",
    default:
      "Do not refer to yourself in your answers. Do not say as an AI language model，默认用中文回复！",
  ...
  },
  {
    ...
    description:
      "控制模型输出的随机性。较高的值会使输出更加多样化和创意性，较低的值会使输出更加集中和确定性。建议创意性任务使用0.9，明确答案的任务使用0。", // "Controls output randomness. Higher for creativity, lower for consistency"
  },
  {
    key: "openAIMaxTokens",
    type: "string",
    ...
    description:
      "控制每次响应的最大长度限制。对于复杂任务（如描述多个人物），建议设置为8000或更高。gpt-4o-mini模型支持最大128k tokens。", // "Controls the maximum length of each response"
  },
  {
    key: "dalleImageSize",
    type: "string",
  ...
  },
  {
    ...
    description: "选择使用的DALL-E模型版本，默认dall-e-3。", // "Choose DALL-E model version: dall-e-2 or dall-e-3"
  },
  {
    key: "dalleQuality",
    type: "string",
    ...
    description:
      "生成图像的质量等级。'hd'模式会生成更细致和连贯的图像，默认为'standard'标准模式。", // "Quality level of generated images. 'hd' for higher detail, 'standard' by default"
  },
  {
    key: "dalleStyle",
    type: "string",
    ...
    description:
      "生成图像的风格选择。'vivid'生动模式会生成更夸张和戏剧性的图像，'natural'自然模式会生成更自然、真实的图像。", // "Style of generated images. 'vivid' for dramatic, 'natural' for realistic"
  },
  {
    key: "shortcutBlock",
    type: "string",
...
export function getOpenaiSettings(): PluginOptions {
  ...
  const chatCompletionEndpoint = logseq.settings![
  ...
  const temperature = Number.parseFloat(
    logseq.settings!["openAITemperature"] as string
  );
  const maxTokens = Number.parseInt(
    logseq.settings!["openAIMaxTokens"] as string
  );
  ...
  const injectPrefix = unescapeNewlines(
    logseq.settings!["injectPrefix"] as string
  );
  ...
  return {
    ...
    openAIMaxTokens: maxTokens, // 4000
    dalleImageSize, // "1024"
    dalleModel, // "dall-e-3"
